...
def check_across_seeds(accs, f1s, result_df, num_classes=20):
    ...
    assert torch.all(torch.abs(accs[1:] - accs[:1]) < 1e-1) and torch.all(
        torch.abs(f1s[1:] - f1s[:1]) < 1e-1), "test results are not compatible \n{}\n{}".format(accs, f1s)
...
if __name__ == "__main__":
    ...
    CLF.load_state_dict(torch.load('./model_state.pth', map_location=torch.device('cpu')))
